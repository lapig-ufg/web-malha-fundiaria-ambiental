...
async def execute_queries(controller_name: str, method_name: str, params: dict):
    try:
        ...
        if method_name not in all_queries:
            return {}
...
async def get_chart_data(request: Request):
    # Extract method name from path (e.g., /service/charts/resumo -> resumo)
    method_name = request.url.path.strip('/').split('/')[-1]
    ...
    return await execute_queries("charts", method_name, params)

async def get_map_data(request: Request):
    method_name = request.url.path.strip('/').split('/')[-1]
    params = dict(request.query_params)
    params.update(request.path_params)
    return await execute_queries("map", method_name, params)
